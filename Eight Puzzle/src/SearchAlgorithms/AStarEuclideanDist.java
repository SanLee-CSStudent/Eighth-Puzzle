...
public class AStarEuclideanDist extends Search{

	public AStarEuclideanDist(State i, State g, Operators o) {
		super(i, g, o);

	}
	
	private double EuclideanDist(State curr) {
		double h = 0;
		
		for(int i = 0; i < curr.size; i++) {
			for(int j = 0; j < curr.size; j++) {
				// ignore blank space = 0
				if(curr.state[i][j].equals("0")) {
					continue;
				}
				double width=0, height=0, distance=0;
				String currNum = curr.state[i][j];
				
				int[] distF = this.goalState.findPos(currNum);
				width = distF[0] - i;
				height = distF[1] - j;
	...
	public void run() {
		...
		Node root = new Node(initialState, 0, 0, null, EuclideanDist(initialState));
		numNodes++;
	...
	PriorityQueue<Node> expand(PriorityQueue<Node> pq, Node curr, ArrayList<Node> set) {
		...
		for(int i = 0; i < 4; i++) {
			if(directions[i] != null) {
				...
				int g = curr.depth + 1;
				double h = this.EuclideanDist(directions[i]);
